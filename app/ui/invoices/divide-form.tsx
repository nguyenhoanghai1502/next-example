  ...
  return (
    <form action={handleDivideProfit}>
        ...
        <div className="mb-4 hidden">
          ...
          <div className="relative">
            <select
              ...
              name="profit_id"
              className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
              defaultValue={profit.id}
            >
              <option value="" disabled>
                Chọn lợi nhuận
              </option>
              {profit_list.map((profit: any) => (
        ...
        <div className='my-2'>
          <div className="w-full bg-gray-200 rounded-full h-1.5 mb-2">
            <div
              ...
              style={{
                width: Math.floor((profit.total_profit / Math.max(profit.amount, 1)) * 100) + "%"
              }}
            ></div>

          </div>
          <div className="flex justify-between mb-1">
            <span className="text-xs font-medium text-black ">{formatCurrency(profit.total_profit)}</span>
            <span className="text-xs font-medium text-black ">{formatCurrency(profit.remaining_profit)}</span>
          </div>
        </div>
        <div className="mb-4">
      ...
      <div className="mt-6 flex justify-center gap-4">
        ...
        <Button type="submit">Chia lợi nhuận</Button>
      </div>
    </form>
  );
}
